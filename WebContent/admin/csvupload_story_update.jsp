			...
			if(csv.toString().equals(filepath+"story.csv")) {
				...
				while((line = br.readLine()) != null) {
					...
					if(i == 0) {
						...
						pstmt = conn.prepareStatement("insert into story (Story_id,csvfilename,title,writer,summary,category_id,imgname,recommend,totalcount,director,sort,engname,comment,col1,col1content,col2,col2content,col3,col3content,photobookid,rankchnum) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
						
						for (String output: token) {
							output = output.replaceAll("\"", "");
							
							// int
							if(index == 6 || index == 8 || index == 9 || index == 11||index==20||index==21) {
