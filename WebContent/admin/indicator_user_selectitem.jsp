...
<H2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;이야기별 선택지 구매 횟수</H2>
<section>
	<H3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;이야기별 선택지 구매 횟수</H3>
</section>
	<%
	String filepath = "";
	String filename = "";
	...
	<form action="indicator_user_selectitem.jsp" method="post">
		...
		<input type="hidden" name="type" value="selectitem">
	</form>
	...
	try {
		if(type != null) {
			...
			<form action="csvfiledownload.jsp" method="post">
				<input type="hidden" name="filename" value="user_selectitem">
			...
			filename = "user_selectitem.csv";
			fw = new FileWriter(filepath+filename);

			conn = ConnectionProvider.getConnection("logdb");
			pstmt = conn.prepareStatement("select date_format(regdate, '%Y-%m-%d') m, storyid from info_episode where regdate between ? and ?");
			...
			conn = ConnectionProvider.getConnection("afgt");
			pstmt = conn.prepareStatement("select date_format(input_date, '%Y-%m-%d') m,storyid,count(uid) from user_selectitem where input_date between ? and ? group by storyid,m");
			pstmt.setTimestamp(1, start);
			pstmt.setTimestamp(2, end);
			rs = pstmt.executeQuery();
			...
			for(int i = 0; i<list.size();i++) {
				...
				fw.append(',');
				fw.append(String.valueOf(list.get(i).count * 10));
				fw.append('\n');
