...
<%
	...
	try {
		...
		while(files.hasMoreElements()) {
			...
			// 1.업로드 파일 이름 체크
			if(csv.toString().equals(filepath+"attendevent.csv")) {
					...
					if(i == 0) {
						...
						if(rs.next()) {
							...
							if(colcount != filecolcount) {
								...
								pstmt = conn.prepareStatement("truncate attendEvent");
				...
				if(completeCount == i) {
					out.print("출석 보상 데이터 적용 완료!"); %> <br> <%
				}
				else {
					out.print("출석 보상 데이터 적용 실패! 확인해주세요."); %> <br> <%
				}
			}
			else {
				out.print("파일명이 다릅니다. 업로드한 파일이 attendevent.csv가 맞는지 확인해주세요!"); %> <br> <%
				break;
			}
