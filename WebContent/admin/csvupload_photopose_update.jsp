...
<%
	...
	try {
		...
		while(files.hasMoreElements()) {
			...
			// 1.업로드 파일 이름 체크
			if(csv.toString().equals(filepath+"photopose.csv")) {
				...
				while((line = br.readLine()) != null) {
					...
					if(i == 0) {
						pstmt = conn.prepareStatement("select count(*) from information_schema.columns where table_name = 'photopose'  and table_schema = 'idol'");
						...
						if(rs.next()) {
							...
							if(colcount != filecolcount) {
								...
								pstmt = conn.prepareStatement("truncate photopose");
						...
						pstmt = conn.prepareStatement("insert into photopose (ppid,ppname,thumbnail,ppaniname) values(?,?,?,?)");
				...
				out.print("파일명이 다릅니다. 업로드한 파일이 photopose.csv가 맞는지 확인해주세요!"); %> <br> <%
				break;
			}
