...
<%
	...
	try {
		...
		while(files.hasMoreElements()) {
			...
			// 1.업로드 파일 이름 체크
			if(csv.toString().equals(filepath+"AssetBundleVersion.csv")) {
				...
				while((line = br.readLine()) != null) {
					...
					if(i == 0) {
						pstmt = conn.prepareStatement("select count(*) from information_schema.columns where table_name = 'AssetBundleVersion'  and table_schema = 'idol'");
						...
						if(rs.next()) {
							...
							if(colcount != filecolcount) {
								...
								pstmt = conn.prepareStatement("truncate AssetBundleVersion");
						...
						pstmt = conn.prepareStatement("insert into AssetBundleVersion (AssetBundleName, version) values(?,?)");
				...
				if(completeCount == i) {
					out.print("에셋 번들 버전 데이터 적용 완료!"); %> <br> <%
				}
				else {
					out.print("에셋 번들 버전 데이터 적용 실패! 확인해주세요."); %> <br> <%
				}
			}
			else {
				out.print("파일명이 다릅니다. 업로드한 파일이 AssetBundleVersion.csv가 맞는지 확인해주세요!"); %> <br> <%
				break;
			}
