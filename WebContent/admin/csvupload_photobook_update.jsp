...
<%
	...
	try {
		...
		while(files.hasMoreElements()) {
			...
			// 1.업로드 파일 이름 체크
			if(csv.toString().equals(filepath+"photobook.csv")) {
				...
				while((line = br.readLine()) != null) {
					...
					if(i == 0) {
						pstmt = conn.prepareStatement("select count(*) from information_schema.columns where table_name = 'photobook'  and table_schema = 'idol'");
						...
						if(rs.next()) {
							...
							if(colcount != filecolcount) {
								...
								pstmt = conn.prepareStatement("truncate photobook");
						...
						pstmt = conn.prepareStatement("insert into photobook (photobookid,photobookname,col1,col1content,col2,col2content,col3,col3content,col4,col4content,col5,col5content,photopage1id,photopage2id,photopage3id,chid,chnum) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				...
				out.print("파일명이 다릅니다. 업로드한 파일이 photobook.csv가 맞는지 확인해주세요!"); %> <br> <%
				break;
			}
